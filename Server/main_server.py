...
def handle_passenger(client_socket, addr):
    ...
    passengers[addr] = {"name": name, "lat": None, "long": None}
    ...
    passengers[addr]["lat"] = lat
    passengers[addr]["long"] = lon
    ...
    if matched_driver:
        ...
        drivers[driver_id]["available"] = False
    else:
...
def handle_driver(client_socket):
    client_socket.send("Enter your name: ".encode('utf-8'))
    name = client_socket.recv(1024).decode('utf-8')
...
def server():
    ...
    while True:
        ...
        if client_type == 'passenger':
            ...
            threading.Thread(target=handle_driver, args=(client_socket,)).start()
        else:
